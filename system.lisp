      ...
      (labels ((copy (from to)
                 ...
                 (dolist (file (append (uiop:directory-files from)
                   ...
                   (if (uiop:directory-pathname-p file)
                       (copy file (relative-dir to (car (last (pathname-directory file)))))
                       (uiop:copy-file file (make-pathname :directory (pathname-directory to) :defaults file))))))
        (copy (first (asdf:output-files 'download-op system))
              (relative-dir (first (asdf:output-files op system)) "lib")))
      (call-next-method)))
