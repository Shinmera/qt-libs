...
(defmethod stage :around (stage (library foreign-library) &rest args &key &allow-other-keys)
  (restart-case
      (progn
...
(defmethod stage ((stage (eql :clean)) (library foreign-library) &key &allow-other-keys)
...
(defmethod stage :around ((stage (eql :download-sources)) (library download-library) &key force &allow-other-keys)
  (when (or force (not (uiop:file-exists-p (sources-archive library))))
    (ensure-directories-exist (base-directory library))
    (call-next-method)))

(defmethod stage :around ((stage (eql :download-binaries)) (library download-library) &key force &allow-other-keys)
  (when (or force (not (uiop:file-exists-p (binaries-archive library))))
    (ensure-directories-exist (base-directory library))
    (call-next-method)))

(defmethod stage ((stage (eql :download-sources)) (library download-library) &key &allow-other-keys)
  (download-file (sources-url library)
                 (sources-archive library)))

(defmethod stage ((stage (eql :prepare-sources)) (library download-library) &key force &allow-other-keys)
...
(defmethod stage ((stage (eql :download-binaries)) (library download-library) &key (platform (platform)) (arch (arch)) &allow-other-keys)
  (download-file (binaries-url library :platform platform :arch arch)
                 (binaries-archive library)))

(defmethod stage ((stage (eql :install-binaries)) (library download-library) &key force &allow-other-keys)
...
(defmethod stage :after ((stage (eql :clean)) (library download-library) &key archives &allow-other-keys)
...
(defmethod stage :around ((stage (eql :prepare-sources)) (library build-library) &key force &allow-other-keys)
  (when (or force (not (uiop:directory-exists-p (build-directory library))))
    (call-next-method)))

(defmethod stage :around ((stage (eql :compile-sources)) (library build-library) &key force &allow-other-keys)
...
(defmethod stage :around ((stage (eql :install-sources)) (library build-library) &key force &allow-other-keys)
...
(defmethod stage ((stage (eql :prepare-sources)) (library make-build-library) &key &allow-other-keys)
  (when (next-method-p) (call-next-method)))

(defmethod stage ((stage (eql :compile-sources)) (library make-build-library) &key &allow-other-keys)
...
(defmethod stage ((stage (eql :install-sources)) (library make-build-library) &key &allow-other-keys)
  (check-file-exists (merge-pathnames (make-file library) (build-directory library)))
  (with-chdir ((build-directory library))
...
(defmethod stage ((stage (eql :prepare-sources)) (library cmake-build-library) &key &allow-other-keys)
  (when (next-method-p) (call-next-method))
...
(defmethod expected-checksum ((library checksummed-library) (archive (eql :sources)) &key &allow-other-keys)
  (getf (checksums library) :sources))

(defmethod expected-checksum ((library checksummed-library) (archive (eql :binaries)) &key (platform (platform)) (arch (arch)) &allow-other-keys)
  (third (find (list platform arch) (checksums library) :key #'butlast :test #'equal)))

(defmethod stage :after ((stage (eql :download-sources)) (library checksummed-library) &key &allow-other-keys)
...
(defmethod stage :after ((stage (eql :download-binaries)) (library checksummed-library) &key platform arch &allow-other-keys)
...
(defmethod stage ((stage (eql :download-sources)) (library locally-available-library) &key &allow-other-keys))

(defmethod stage ((stage (eql :prepare-sources)) (library locally-available-library) &key &allow-other-keys)
  (unless (locally-available-p library)
    (error "~a cannot be found locally." (name library))))

(defmethod stage ((stage (eql :install-sources)) (library locally-available-library) &key &allow-other-keys)
